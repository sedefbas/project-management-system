package management.sedef.minio.controller;
...
public class MinioController {
    ...
    @GetMapping("/getObjectUrl/{bucketName}/{objectName}")
    public String getObjectUrl(@PathVariable String bucketName, @PathVariable String objectName) {

        LOGGER.info("MinioController | getObjectUrl is called");

        LOGGER.info("MinioController | getObjectUrl | bucketName : " + bucketName);
        LOGGER.info("MinioController | getObjectUrl | objectName : " + objectName);

        String url = minioService.getObjectUrl(bucketName, objectName);

        if (url != null) {
            return url;  // URL'yi frontend'e gönderiyoruz
        } else {
            throw new FileResponseException("Object not found in MinIO");
        }
    }
}
